...
@section('content')
    ...
    <div class="site-blocks-cover inner-page-cover overlay"
        style="background-image: url({{ asset('assets/images/' . $propDetaile->bgimage . '') }});" data-aos="fade"
            ...
            <div class="row">
                        ...
                        <div class="row no-gutters mt-5">
                            ...
                            @foreach ($propImages as $gallery)
                                <div class="col-sm-6 col-md-4 col-lg-3">
                                    <a href=" {{ asset('assets/images/' . $gallery->image . '') }}"
                                        class="image-popup gal-item"><img
                                            src=" {{ asset('assets/images/' . $gallery->image . '') }}" alt="Image"
                                            class="img-fluid"></a>
                                </div>
                            @endforeach
                ...
                <div class="col-lg-4">
                    ...
                    <div class="bg-white widget border rounded">
                        ...
                        <form method="post" action="{{ route('make.request', $propDetaile->prop_id) }}"
                            class="form-contact-agent">
                            @csrf


                            <input type="text" id="prop_id" name="prop_id" value="{{ $propDetaile->prop_id }}"
                                class="form-control" hidden required>
                            <input type="text" id="agent_name" name="agent_name" value="{{ $propDetaile->agent_name }}"
                                class="form-control" hidden required>
                    ...
                    <div class="bg-white widget border rounded">
                        ...
                        <div class="px-3" style="margin-left: -15px;">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ route('prop.detaile', $propDetaile->prop_id) }}&quote={{ $propDetaile->title }}"
                    ...
                    @if (\Session::has('saved-success'))
                    <div class="alert alert-success">
                        <p>{!! \Session::get('saved-success') !!}</p>
                    </div>
                @endif
                
                <div style="margin-left: 70px;" class="form-group">
                    @if ($isSaved)
                        <input type="submit" class="btn btn-secondary" value=Saved" disabled>
                    @else
                        <a href="{{ route('save.prop', $propDetaile->prop_id) }}">
                            ...
                            <div class="col-md-6 col-lg-4 mb-4">
                                ...
                                <div class="property-entry h-100">
                                    <a href="{{ route('prop.detaile', $relprop->prop_id) }}" class="property-thumbnail">
                                        ...
                                        <img src="{{ asset('assets/images/' . $relprop->bgimage . '') }}" alt="Image"
                                            class="img-fluid">
                                    </a>
                                    <div class="p-4 property-body">
                                        ...
                                        <h2 class="property-title"><a
                                                href="property-details.html">{{ $relprop->title }}</a></h2>
                                        <span class="property-location d-block mb-3"><span
                                                class="property-icon icon-room"></span>{{ $relprop->address }}</span>
                                        <strong
                                            class="property-price text-primary mb-3 d-block text-success">{{ $relprop->price }}</strong>
                                        <ul class="property-specs-wrap mb-3 mb-lg-0">
                                            <li>
                                                <span class="property-specs">Beds</span>
                                                <span class="property-specs-number">{{ $relprop->beds }}
                                                    <sup>+</sup></span>
